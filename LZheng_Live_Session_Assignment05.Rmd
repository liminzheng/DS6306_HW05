---
title: "LZheng_LiveSessionAssignment05"
author: "Limin Zheng"
...
colnames(df) <- c("Name","Gender", "Quantity") 
```
...
```{r}
y2016 <- df[which(df$`Name`!= "Fionayyy"), ] 
```
...
colnames(y2015) <- c("Name","Gender", "Quantity") 
```
...
From the last ten names picked, it shows that most parents don't like name starts with a "Z", especially for a girl's name.
...
```{r}
final$Total <- 0
final$Total <- paste(final$Quantity.x + final$Quantity.y)
...
final_sorted = final_sorted[, c(1,2,5)]
...
```{r}
final_sorted_girl = final_sorted[order(final_sorted$Gender),]
head(final_sorted_girl, 10)
```

d.Write these top 10 girl names and their Totals to a CSV file.  Leave out the other columns entirely.
```{r}
row.names(final_sorted_girl) <- NULL
write.csv(final_sorted_girl[c(1:10), c(1, 3)], file="top10girlname.csv")
```
...
```{r}
#library(dataMaid)
#makeCodebook(final_sorted, vol = "", reportTitle = NULL, file = NULL)
#makeCodebook(final_sorted_girl, vol = "", reportTitle = NULL, file = NULL)
# Github: https://github.com/liminzheng/MSDS6306HW05
```
